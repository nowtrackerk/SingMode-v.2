...
const ParticipantView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const timeoutTimer = setTimeout(() => {
      if (!session || !participant) {
        console.log("[Participant] Connection timeout, showing reconnect options.");
        setShowConnectTimeout(true);
      }
    }, 8000);

    // Proactive State Request (Ask host if we are waiting too long)
    const requestPulse = setInterval(() => {
      if (!session || !participant) {
        console.log("[Participant] Proactively requesting state...");
  ...
  */
  ...
  const showLogin = !participant && !userProfile && !isJoinedViaQR;
            ...
            <button
              onClick={async () => {
                ...
                if (room) {
                  ...
                  await initializeSync('PARTICIPANT', room);
                  await refresh();
                } else {
      ...
      {showSessionScanner && (
        <div className="fixed inset-0 bg-black/95 flex items-center justify-center p-8 z-[200] animate-in zoom-in-95 duration-300">
          <div className="w-full max-w-4xl text-center relative bg-[#050510] border-4 border-white/10 rounded-[3rem] p-4 md:p-10 overflow-hidden max-h-[90vh] overflow-y-auto">
            ...
            <h3 className="text-4xl md:text-5xl font-black text-white uppercase tracking-tight mb-8 font-bungee neon-glow-green mt-8">RECONNECT</h3>

            <SessionList
              onJoin={(id) => {
                window.location.search = `? room = ${id} `;
              }}
              filterHostName="SingMode DJ"
            />
          </div>
        </div>
      )}
